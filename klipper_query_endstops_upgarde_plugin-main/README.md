# Klipper query_endstops macros upgarde plugin

Этот плагин добавляет команду ```QUERY_ENDSTOPS_SILENT```, которая обновляет последнее значения концевиков, без вывода в консоль. Это необходимо для работы Тулченджера и 3-х осевого датчика калибровки сопел, без засорения консоли.

# Instalation
- *Connect via ssh*
```
cd ~
git clone https://github.com/RedFabLab/klipper_query_endstops_upgarde_plugin
...
class QueryEndstops:
    ...
    ##############################################################################
    ############  NEW NEW NEW NEW NEW NEW ########################################
    # TODO: update only Z endstop.
    cmd_QUERY_QUERY_ENDSTOPS_SILENT_help = "Update endstops last state without print result"
    def cmd_QUERY_ENDSTOPS_SILENT(self, gcmd):
        # Query the endstops
        print_time = self.printer.lookup_object('toolhead').get_last_move_time()
        self.last_state = [(name, mcu_endstop.query_endstop(print_time))
                           for mcu_endstop, name in self.endstops]
        # logging.error("QUERY_ENDSTOPS_SILENT. self.last_state = " + str(self.last_state))
        # Report results
        # msg = " ".join(["%s:%s" % (name, ["open", "TRIGGERED"][not not t])
        #                 for name, t in self.last_state])
        # gcmd.respond_raw(msg)
